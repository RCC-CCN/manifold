...
std::pair<Vec<Halfedge>, Vec<vec3>> QuickHull::buildMesh(double epsilon) {
  ...
  std::for_each(halfedges.begin(), halfedges.end(), [&](Halfedge& he) {
    he.pairedHalfedge = mapping[he.pairedHalfedge];
  });
  ...
  std::for_each(countAt(0_uz), countAt(originalVertexData.size()),
                [&](size_t i) {
